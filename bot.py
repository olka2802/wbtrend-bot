...
async def start_handler(message: types.Message, state: FSMContext):
    keyboard = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="Анализ ниши")], [KeyboardButton(text="Помощь")]],
        resize_keyboard=True
    )
    await message.answer("Привет! Я бот, который поможет тебе выбрать нишу на маркетплейсе Wildberries.", reply_markup=keyboard)
    await message.answer("Нажми 'Анализ ниши', чтобы начать.")

@router.message(F.text == "Анализ ниши")
async def analyze_niche(message: types.Message, state: FSMContext):
    await message.answer("Введите категорию товаров (например: одежда, косметика, товары для дома и т.д.)")
...
async def help_handler(message: types.Message):
    await message.answer("Для начала анализа нажмите 'Анализ ниши'.")
